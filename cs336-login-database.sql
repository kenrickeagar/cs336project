...
CREATE TABLE Bids(amount int, closing_date datetime, time_of_bid datetime, buyer_id int, seller_id int, i_id int, primary key (buyer_id,i_id), foreign key (buyer_id) references users (id),foreign key (seller_id) references users (id), foreign key (i_id) references items (i_id));
