...
CREATE TABLE Auto_Bids(id int, i_id int, prev_price float, max_amount float, increment float, foreign key (id) references Users (id),foreign key (i_id) references Items (i_id));
